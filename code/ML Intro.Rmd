...
mixturesWithFeatures <- read.csv(here("data", "raw", "MixturesWithFeatures.csv"))
#mixturesWithFeatures <- read.csv("data/")
...
xgbLinearFit<-train(experimental_values ~ diff_mixture_size + overlap_percent + angle_dist, data = train_set,
             method = "xgbLinear",
             trControl = fitControl,
             metric="RMSE")
#Look at performance in cross-validation
...
#Now try it on the test set
mixture.predict = predict(xgbLinearFit, test_set)
mixture.results <- cbind(test_set,Predicted=mixture.predict)
...
mixture.results %>%
  ...
  geom_point() +
  geom_smooth(method = "lm")+
  labs(x="Percentage discrimination",y="XGBoost Model")

fit3 <- lm(mixture.predict ~ experimental_values, data = mixture.results)

print(paste0("Adj R2 = ",signif(summary(fit3)$adj.r.squared, 5)))
print(paste0(" P =",signif(summary(fit3)$coef[2,4], 5)))
```

# Aubrey - ML Model Testing
```{r Aubrey's ML Model Testing}

# Install and load monmlp package
pacman::p_load(monmlp)

# Monotone Multi-Layer Perceptron Neural Network
monmlpFit<-train(experimental_values ~ diff_mixture_size + overlap_percent + angle_dist, data = train_set,
             method = "monmlp",
             trControl = fitControl,
             metric="RMSE")
...
# Now trying it on the test set
mixture.predict = predict(monmlpFit, test_set)
mixture.results <- cbind(test_set,Predicted=mixture.predict)
...
mixture.results %>%
  ...
  geom_point() +
  geom_smooth(method = "lm")+
  labs(x="Percentage discrimination",y="Monotone Multi-Layer Perceptron Neural Network")

fit_monmlpFit <- lm(mixture.predict ~ experimental_values, data = mixture.results)

print(paste0("Adj R2 = ",signif(summary(fit_monmlpFit)$adj.r.squared, 5)))
print(paste0(" P =",signif(summary(fit_monmlpFit)$coef[2,4], 5)))

#comment
```

# lily - ML Model Testing
```{r lily's ML Model Testing}

# Install and load ctree package
pacman::p_load(ctree)

# ctree
ctreeFit<-train(experimental_values ~ diff_mixture_size + overlap_percent + angle_dist, data = train_set,
             method = "ctree",
             trControl = fitControl,
             metric="RMSE")
...
# Now trying it on the test set
mixture.predict = predict(ctreeFit, test_set)
mixture.results <- cbind(test_set,Predicted=mixture.predict)
...
fit_ctreeFit <- lm(mixture.predict ~ experimental_values, data = mixture.results)

print(paste0("Adj R2 = ",signif(summary(fit_ctreeFit)$adj.r.squared, 5)))
print(paste0(" P =",signif(summary(fit_ctreeFit)$coef[2,4], 5)))
